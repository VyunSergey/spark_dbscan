...
class FindClosePointsSuite extends SuiteBase {
  ...
  val dataset11: RDD[Point] = sc.parallelize(Array( new Point(0.0, 0.0), new Point(1.0, 0.0),
    new Point(0.5, 1.0), new Point(0.5, 0.5), new Point(0.5, 5.0)))

  val settings: DbscanSettings = new DbscanSettings().withEpsilon(0.4)
  val partitionedData: PointsPartitionedByBoxesRDD = PointsPartitionedByBoxesRDD(dataset1, dbscanSettings = settings)
  val boxes: Iterable[Box] = partitionedData.boxes
  val boundingBox: Box = partitionedData.boundingBox
  ...
  test("DistanceAnalyzer should find close points properly") {
    ...
    val partitionedAndSortedData = PointsPartitionedByBoxesRDD(dataset11, dbscanSettings = settings)
    val distanceAnalyzer = new DistanceAnalyzer(settings)
