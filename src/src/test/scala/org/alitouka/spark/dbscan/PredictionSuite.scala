...
class PredictionSuite extends DbscanSuiteBase {
  val clusteredPoints: RDD[Point] =
    sc.parallelize(Array(
      create2DPoint(0.0, 0.0).withClusterId(1),
  ...
  val settings1: DbscanSettings = new DbscanSettings().withEpsilon(1.5).withNumberOfPoints(3)
  val settings2: DbscanSettings = new DbscanSettings().withEpsilon(1.5).withNumberOfPoints(4).withTreatBorderPointsAsNoise(true)

  val model1: DbscanModel = new DbscanModel(clusteredPoints, settings1)
  val model2: DbscanModel = new DbscanModel(clusteredPoints, settings2)
  ...
  test("DbscanModel should mark an ambiguous point as noise") {
    val predictedClusterId = model2.predict(create2DPoint(4.5, 0.5))
