...
class BoxCalculatorSuite extends SuiteBase {
  val dataset1: RDD[Point] = sc.parallelize(
    Array(
      // Border points
      new Point(2.5, 0), new Point(0, 1.5), new Point(4, 0.5), new Point(2.5, 2.0),
      // Close points within one box
      new Point(0.2, 0.2), new Point(0.2, 0.7), new Point(0.7, 0.2),
      // Close points in different boxes
      new Point(2.8, 0.8), new Point(3.3, 0.8), new Point(2.8, 1.3))
  )

  test("BoxCalculator should generate 2-level hierarchy of boxes") {
    val dbscanSettings = new DbscanSettings().withEpsilon(1)
    // Despite 8-level settings,
    val partitioningSettings = new PartitioningSettings(2, 8)

    // only 2 levels should be generated
    // because boxes will become too small at deeper levels
  ...
  test("BoxCalculator should put 5 points into 3 boxes") {
    ...
    val boxTreeRoot = BoxCalculator.generateTreeOfBoxes(rootBox, partitioningSettings, dbscanSettings)
    val points = Array(
      new Point(1.0, 1.5), new Point(3.0, 0.5), new Point(4.5, 0.5),
  ...
  test("BoxCalculator should generate embracing box") {
    ...
    // Id is less than root box id, so it should not be included into the embracing box
    val boxOnRight = new Box((1.0, 2.0), (0.0, 1.0)).withId (9)
