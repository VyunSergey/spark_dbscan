...
private [dbscan] class CommonArgsParser[C <: CommonArgs](val args: C, programName: String) extends scopt.OptionParser[Unit](programName) {
  ...
  opt[String]("distanceMeasure")
    .foreach {
      x => args.distanceMeasure = Class.forName(x).asInstanceOf[DistanceMeasure]
    }

  opt[String]("ds-debugOutput")
    .foreach { x => args.debugOutputPath = Some(x) }
}
